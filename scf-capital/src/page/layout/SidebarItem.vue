  ...
  <div class='menu-wrapper'>
    <template v-for="item in routes">
      ...
      <router-link v-if="!item.hidden&&!item.noDropdown&&item.isHome&&item.children&&item.children.length>0" :to="item.path+item.children[0].path">
        <el-menu-item :index="item.path" class='submenu-title-noDropdown' style="background:white;text-align: center;background-color: #dfdfdf;">
          <img style="width: 20px;" :src="userLogo" alt="logo">
          <span class="logo" style="margin-left: 0;">企小融资方管理后台</span>
        </el-menu-item>
      </router-link>
...
<script>
import userLogo from '@/assets/logo.svg'
export default {
  name: 'SidebarItem',
  data () {
    return {
