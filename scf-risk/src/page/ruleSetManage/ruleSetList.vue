<template>
  <div class="page-container" ref="page-container" id="page-container">
    <div class="filter-container" ref="filter-container" id="filter-container">
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        <el-form-item label="规则集名称" prop="rulesetName">
          <el-input v-model="listQuery.rulesetName" style="width:120px;"></el-input>
        </el-form-item>
        <el-form-item label="规则集类型" prop="dataCategory" class="query-select" style="width:220px;">
          <el-select v-model="listQuery.dataCategory" style="width:120px;">
            <el-option label="全部" value=""></el-option>
            <el-option label="个人信用 " value='PERSON_CREDIT'></el-option>
            <el-option label="企业信用 " value='ENTERPRISE_CREDIT'></el-option>
            <el-option label="企业经营 " value='ENTERPRISE_MANAGE'></el-option>
          </el-select>
        </el-form-item>
    ...
    <div class="tb_box" :style="{height:tableMaxHeight + 'px'}">
	    <el-row style="background-color: #dddee2;line-height: 40px;color: #303133;font-size: 13px;">
		  	...
		  	<el-col :span="6" style="text-align: right;padding-right: 30px;">
		  		<router-link to="ruleSetNew">
		  			<el-button size="small" type="primary"><i class="iconfont icon-reset"></i>新建规则集</el-button>
          </router-link>
		  	</el-col>
			</el-row>
				    	...
				    	<div class="content_collapse">
				    		<el-radio-group v-model="selectRedio" style="width: 100%;">
					    		...
					    		<el-table :data="item.list" @current-change="handleSelectionChange" highlight-current-row :max-height=tableMaxHeight>
									...
									<el-table-column prop="rulesetVersion" label="版本" min-width="80">
									</el-table-column>
									...
									<el-table-column prop="rulesetStatus" label="启用/禁用" width="220">
										<template slot-scope="scope">
											<el-switch
													@change="changeRuleset(scope.row.rulesetCode,scope.row.rulesetVersion,scope.row.rulesetStatus,index)"
											    v-model="scope.row.rulesetStatus"
											    :disabled="scope.row.rulesetStatus==='dis'?true:false"
											    active-color="#13ce66"
											    active-value="ENABLED"
											    inactive-value="DISABLED">
										  	</el-switch>
										  	<!--<el-radio :label="scope.row.rulesetStatus" @change="radioChange"></el-radio>-->
										</template>
									</el-table-column>

									<el-table-column prop="rulesetCode" label="操作" width="200" align="center">
										<template slot-scope="scope"  v-if="!roleHasPermissions['loan_manage_loan_list_view']">
											<span class="cursor_a router_link" @click.stop="addVersion(scope.row.rulesetCode,scope.row.rulesetName,scope.row.rulesetVersion)">
												新增版本
											</span>
											<span>
												<router-link :to="'ruleSetDetail/' + scope.row.rulesetCode+'/'+ scope.row.rulesetVersion"  class="router_link">查看详情</router-link>
											</span>
										</template>
									</el-table-column>
    ...
    <!--新增版本弹窗-->
    <el-dialog title="新建规则集版本" :visible.sync="dialogFormAddRulesetVersion" style="width: 1000px;margin: auto;">
		  <el-form :model="formRulesetVersion"  ref="formRulesetVersion" label-width="100px" >
		    ...
		    <el-form-item
		    	label="所基于版本"
		    	prop="rulesetVersion">
			    <el-input  v-model="formRulesetVersion.rulesetVersion" disabled></el-input>
		    </el-form-item>
		    <el-form-item
		    	label="描述"
		    	prop="rulesetDescription">
			    <el-input type="textarea" v-model="formRulesetVersion.rulesetDescription"></el-input>
		    </el-form-item>
		  </el-form>
...
<script>
  ...
  import { getListData,addVersion,enableDisableRuleset } from '@/api/ruleSetManage/ruleSetList'
  ...
  export default {
    ...
    methods: {
      ...
      handleChange(val) {
	    ...
	    addVersion(rulesetCode,rulesetName,rulesetVersion){//新增版本
	    		let that = this;
	    ...
	    submitFormGrantPermission(){//保存新增;
      	addVersion(this.formRulesetVersion).then(res =>{
    			if (res.data.code === '200') {
    				this.getList();
...
//		        this.resetChecked();//清空树结构
      ...
      resetFormUpdateRole(formName) {//取消新增
	    ...
	    changeRuleset(rulesetCode,rulesetVersion,statusName,index){//启用/禁用
	    	let data = {
	    		rulesetCode,
	    		rulesetVersion,
	    		statusName
