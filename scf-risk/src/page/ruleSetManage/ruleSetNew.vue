<template>
	<div class="page-container" style="overflow-y: auto;">
		<div class="container_box">
			<el-form :model="detailDataForm" ref="detailDataForm" label-width="190px" size="mini" label-position="right">
		    	<div class="info-list">
				    ...
				    <el-form-item label="规则集编号：">
				      	<el-input class="must_input" style="width: 300px;" v-model="detailDataForm.rulesetCode" placeholder="">
				       	</el-input>
				    </el-form-item>
				    <el-form-item label="规则集版本：">
				      	<el-input class="must_input" style="width: 300px;" v-model="detailDataForm.rulesetVersion" disabled placeholder="">
				       	</el-input>
				    </el-form-item>
		    			...
		    			<template>
						  	<el-radio-group v-model="categoryIndex">
							  	<span v-for="(item,index) in queryDataCategoryList" style="padding-right: 20px;">
							  		<el-radio @change="radioChange(index,item.categoryCode)" :label="index" border>{{item.categoryName}}</el-radio>
							  	</span>
						  	</el-radio-group>
						</template>
			      	</el-form-item>
...
//	      		this.$notify.error('2条够啦！')
	    ...
	    saveRuleSet(){
	    	...
	    	if(!data.dataCodes){
	      	...
	      	RuleSetDetail.addRuleset(data).then(res =>{
	      		if(res.data.code === '200'){
	      			this.$store.dispatch('DelVisitedViews', {path: this.$route.path}).then(() => {
		                this.$router.push({ path: '/ruleSetManage/ruleSetDetail/'+this.detailDataForm.rulesetCode+'/'+this.detailDataForm.rulesetVersion});
